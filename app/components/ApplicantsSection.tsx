...
export default function ApplicantsSection() {
  ...
  return (
    <section className="flex flex-col lg:flex-row gap-6">
      {/* Sidebar Filters */}
      <aside className="hidden lg:block w-full lg:w-1/3 xl:w-1/4">
      ...
      <div className="w-full">
        ...
        <div className="hidden lg:flex flex-col gap-4">
          {filteredMaids.map((maid) => (
            <div
              key={maid.id}
              className="flex items-center gap-4 bg-white border border-gray-200 rounded-xl shadow hover:shadow-md transition cursor-pointer overflow-hidden"
              onClick={() => openModal(maid)}
            >
              <div className="w-32 h-32 flex-shrink-0 bg-gray-100 rounded-l-xl overflow-hidden flex items-center justify-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:hidden gap-6">
          {filteredMaids.map((maid) => (
            <div
              key={maid.id}
              className="bg-white border border-gray-200 rounded-xl shadow hover:shadow-md transition cursor-pointer overflow-hidden"
              onClick={() => openModal(maid)}
            >
              <div className="relative w-full h-60 bg-gray-100 rounded-t-xl flex items-center justify-center overflow-hidden">
                {maid.showcasePhotos?.[0] ? (
                  <img
                    src={`${API_BASE}${maid.showcasePhotos[0]}`}
                    alt={maid.fullName}
                ...
                {maid.videos.length > 0 && (
                  <span className="absolute top-2 right-2 bg-purple-600 text-white px-2 py-1 rounded-md text-xs font-semibold">
                    Video
                  </span>
                )}
              </div>
              <div className="p-4 space-y-2 bg-purple-50 rounded-b-xl">
